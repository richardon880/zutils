...
class SkyMap:
    ...
    def _sort_index(self):
        sorted_index = np.flipud(np.argsort(self.hpx)) #return index of values from highest to lowest
        return sorted_index
    
    def get_sorted_credible_levels(self):
        sorted_credible_levels = np.cumsum(self.hpx[self.sorted_index]) #cumulative sum of values in that order
        return sorted_credible_levels
    
    def get_credible_levels(self):
        credible_levels =  np.empty_like(self.sorted_credible_levels) #new array to store
        credible_levels[self.sorted_index] = self.sorted_credible_levels #same values, but in original order
        return credible_levels

    def get_conf_int(self, frac=0.9):
        self.pix_mask = self.credible_levels < frac #bool array if point in sky is above credible level from cumulative sum
    ...
    def check_in_conf_interval(self, ra_vals, dec_vals):
        ...
        theta, phi = self.celest2polar(ra_vals, dec_vals) #need to provide coords in polar form
        pixel_locs = hp.ang2pix(self.nside, theta, phi) #get pixel loc of ra and dec
        in_roi = [self.pix_mask[pix] for pix in pixel_locs] #for each pixel location, check if in conf interval by returning bool mask from pix_mask
...
def square_query_HLEDA(ra, dec, width, height):
    ...
    columns = ['objname', 'pgc', 'ob', 'al1950', 'de1950', 'al2000', 'de2000', 'l2',
       'b2', 'sgl', 'sgb', 'f_astr', 'type', 'agnclass', 'logd25', 'e_logd25',
       'logr25', 'e_logr25', 'pa', 'brief', 'e_brief', 'ut', 'e_ut', 'bt',
       'e_bt', 'vt', 'e_vt', 'it', 'e_it', 'kt', 'e_kt', 'm21', 'e_m21',
       'mfir', 'ube', 'bve', 'vmaxg', 'e_vmaxg', 'vmaxs', 'e_vmaxs', 'vdis',
       'e_vdis', 'vrad', 'e_vrad', 'vopt', 'e_vopt', 'v', 'e_v', 'ag', 'ai',
       'incl', 'a21', 'logdc', 'btc', 'itc', 'ubtc', 'bvtc', 'bri25', 'vrot',
       'e_vrot', 'mg2', 'e_mg2', 'm21c', 'hic', 'vlg', 'vgsr', 'vvir', 'v3k',
       'modz', 'e_modz', 'mod0', 'e_mod0', 'modbest', 'e_modbes', 'mabs',
       'e_mabs']
    col_dtype={'objname': str, 'pgc': float, 'ob':str, 'al1950':float,
       'de1950':float, 'al2000':float, 'de2000':float, 'l2':float,
       'b2':float, 'sgl':float, 'sgb':float, 'f_astr':bool,
       'type':str,
       'agnclass':str, 'logd25': float, 'e_logd25':float,
       'logr25':float, 'e_logr25':float, 'pa':float, 'brief':str,
       'e_brief':str, 'ut':float, 'e_ut':float, 'bt':float,
       'e_bt':float, 'vt':float, 'e_vt':float, 'it':float, 'e_it':float, 'kt':float,
       'e_kt':float, 'm21':float, 'e_m21':float, 'mfir':float,
       'ube':float, 'bve':float, 'vmaxg':float, 'e_vmaxg':float,
       'vmaxs':float, 'e_vmaxs':float, 'vdis':float,
       'e_vdis':float, 'vrad':float, 'e_vrad':float, 'vopt':float,
       'e_vopt':float, 'v':float, 'e_v':float, 'ag':float,
       'ai':float, 'incl':float, 'a21':float, 'logdc':float, 'btc':float, 'itc':float,
       'ubtc':float, 'bvtc':float, 'bri25':float, 'vrot':float,
       'e_vrot':float, 'mg2':float, 'e_mg2':float, 'm21c':float,
       'hic':float, 'vlg':float, 'vgsr':float, 'vvir':float, 'v3k':float,
       'modz':float, 'e_modz':float, 'mod0':float, 'e_mod0':float,
       'modbest':float, 'e_modbes':float, 'mabs':float, 'e_mabs':float}
    ...
    if b' The complete request returns no record.\n' in search_info:
        ...
        return result
    #read the data
    df = pd.read_csv(full_url, header=88, skipfooter=5, engine="python")[11:].reset_index(drop=True)
...
def square_search_bright_stars(ra, dec, width, height):
    ...
    if len(apass) == 0 and len(tycho) == 0:
        ...
        tycho_match = SkyCoord(ra=tycho.RA_ICRS_*u.degree, dec=tycho.DE_ICRS_*u.degree, unit=u.degree)
        apass_match = SkyCoord(ra=apass.RAJ2000*u.degree, dec=apass.DEJ2000*u.degree , unit=u.degree)
...
def search_circ_region(ra, dec, rad, table_name=None, task_name="My Query", keep_table=True):
    ...
    if table_name == None:
        if keep_table == True:
            raise ValueError("table_name must be provided if keep_table is True, stopping search.")
        table_name = "_temp"
...
def search_rect_region(ra1, ra2, dec1, dec2, table_name=None, task_name="My Query"):
    ps_query = make_ps_query("rect", ra=ra1, ra2=ra2, dec=dec1, dec2=dec2, table_name=table_name)
    # ps_query = f"""
...
def rough_search(ralist, declist,
                 point, boxlim):
    """
...
def get_nearest_dists(ralist, declist,
                      point, boxlim):
    """
    ...
    dist_arr = np.sqrt(((ra - ralist_masked)*np.cos(dec))**2 #calculate distances accounting for sky being sphere
...
def get_xmatch_inds(gama_df, ps1_df, searchrad):
    ...
    for row in gama_df.iterrows(): #matching gama to ps1 data, for each gama point
        ind, mask, min_dist = get_nearest_dists( #get the ind, mask and dist to nearest point
        ps1_df.RA.to_numpy(), ps1_df.DEC.to_numpy(), (row[1].RA, row[1].DEC), searchrad
        )
        ps1_ind_match = ps1_df[mask].index[ind] #get the index of the row with nearest point
        gama_ind_match = row[0] #get the index of the row in gama that matched the ps1 data
...
def ray_intersect_edge(point, edge):
    ...
    '''
    a, b = edge[0], edge[1]
    if a[1] > b[1]: #ensure that a is the lower point, this is to check if point outside range
        a, b = b, a
...
def check_in_hull(points, hull):
    ...
    '''
    in_hull = [] #to keep mask for list of points provided True if in hull, false otherwise
    edges = [(a, b) for a, b in pairwise(hull)] #iterate over hull 2 objects at a time to get edges
    for point in points: #for each point
        total = sum(ray_intersect_edge(point, edge) for edge in edges) #check sum of how many intersections
        if total % 2 == 1:
