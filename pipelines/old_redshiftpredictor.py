...
model_filename = "ps_spec_model.sav"

spec_z_model = pickle.load(open(f'/home/richard/Work/CNGFPPR/CNGFPPR/ml_model/{model_filename}', 'rb'))
...
def first_pass_mask(ralist, declist, point, boxlim):
    ra = point[0]
    dec = point[1]
...
def return_model_compatible_testing_data(df):
    df = df[['rMagNormPS', 'grNormPS', 'riNormPS', 'izNormPS', 'zyNormPS']]
    return df


def generate_redshift_predictions(df, bright_stars, gals, n_grid=1000, return_df=False):
    ...
    cdes, zgrid = spec_z_model.predict(training_df, n_grid=n_grid)
    best_z_loc = [list(cde_vals).index(max(cde_vals)) for cde_vals in cdes]
    df["best_z"] = [zgrid[idx][0] for idx in best_z_loc]
    print("Complete.")
    if return_df == True:
        return cdes, zgrid, df
    else:
        return cdes, zgrid
